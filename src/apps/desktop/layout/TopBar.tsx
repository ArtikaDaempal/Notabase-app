  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 shrink-0 relative">
        ...
        {/* 3. USER PROFILE PILL DROPDOWN (Interaktif) */}
        <div
          onClick={() => {
          ...
          className="flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-1.5 shadow-2xs cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/60 transition-colors active:scale-98"
        >
          <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#0B1E48] dark:bg-blue-900 text-[11px] font-bold text-white">
            {initialLetter}
          </div>
          <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{userName}</span>
          <ChevronDown className={cn('h-3.5 w-3.5 text-slate-400 transition-transform', showProfileDropdown && 'rotate-180')} />
        </div>

        {/* Profile Dropdown Menu */}
        {showProfileDropdown && (
          <div className="absolute right-0 top-12 z-50 w-64 rounded-2xl bg-white dark:bg-slate-800 p-2 shadow-xl border border-slate-100 dark:border-slate-700 space-y-1">
            {/* Header User */}
            <div className="p-3 border-b border-slate-100 flex items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#0B1E48] text-white font-extrabold text-xs">
                {initialLetter}
              </div>
              <div className="min-w-0">
                <span className="block text-xs font-bold text-slate-900 truncate">{userName}</span>
                <span className="block text-[10px] text-slate-400 truncate">{accountEmail || 'Akun Microsoft'}</span>
                <span className="inline-block mt-0.5 rounded-full bg-emerald-50 px-1.5 py-0.2 text-[9px] font-bold text-emerald-600">
                  Connected
                </span>
              </div>
            </div>
            ...
            <button
              ...
              className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 text-left transition-colors"
            >
            ...
            >
              ...
              <span>Laporan & Ekspor Excel</span>
            </button>
            ...
            <button
              onClick={() => {
                setShowProfileDropdown(false)
                toast.success(`Sesi pengguna ${userName} berhasil dikunci.`)
              }}
              className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50 text-left transition-colors"
            >
